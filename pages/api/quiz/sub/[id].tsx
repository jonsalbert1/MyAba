...
type Q = {
  id: string;
  prompt: string;             // normalized from question/prompt
  statement?: string | null;  // 👈 shown under the question
    ...
    return (
      <button
        ...
        style={{ width: "100%", textAlign: "left", padding: "10px 12px",
          borderRadius: 10, border: `1px solid ${border}`, background: bg, marginBottom: 8, cursor: revealed ? "default" : "pointer" }}
      >
        <b>{label}.</b> {text}
      </button>
    );
  ...
  return (
    <main style={{ maxWidth: 820, margin: "24px auto", padding: "0 16px", fontFamily: "system-ui,-apple-system,Segoe UI" }}>
      ...
      <section style={{ border: "1px solid #e5e7eb", borderRadius: 12, background: "#fff", padding: 16 }}>
        {/* MAIN QUESTION */}
        <p style={{ marginTop: 0, marginBottom: 8, fontSize: 18 }}>{q.prompt}</p>

        {/* 👇 STATEMENT UNDER THE QUESTION */}
        {q.statement ? (
          <p style={{ marginTop: 0, marginBottom: 12, fontSize: 14, color: "#64748b" }}>
            {q.statement}
          </p>
        ) : null}
